...
export async function GET() {
  ...
  const workspacesWithPermissions = userWorkspaces.map((row: typeof userWorkspaces[number]) => ({
    ...row.workspace,
    role: row.permissionType === 'admin' ? 'owner' : 'member', // Map admin to owner for compatibility
    permissions: row.permissionType,
  }))
...
export async function POST(req: NextRequest) {
  ...
  const auth = await authenticateApiKeyFromHeader(apiKeyHeader, { keyTypes: ['personal'] })
  if (!auth.success || !auth.userId) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  }
  ...
  try {
    ...
    const { name, createAPIKey } = body

    if (!name) {
      return NextResponse.json({ error: 'Name is required' }, { status: 400 })
    }

    const newWorkspace = await createWorkspace(userId, name, createAPIKey)
...
async function createWorkspace(userId: string, name: string, createAPIKey?: boolean) {
  const workspaceId = crypto.randomUUID()
  // const workflowId = crypto.randomUUID()
  const now = new Date()
  let workspaceApiKeyPlain: string | undefined
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      // Optionally create a default API key for the workspace within the transaction
      if (createAPIKey) {
