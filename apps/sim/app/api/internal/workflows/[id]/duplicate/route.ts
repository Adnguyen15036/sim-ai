...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // If a target workspace is provided, ensure the user has write/admin access
    if (workspaceId) {
      const targetPerm = await getUserEntityPermissions(auth.userId, 'workspace', workspaceId)
      if (!(targetPerm === 'admin' || targetPerm === 'write')) {
    ...
    if (error instanceof Error) {
      ...
      if (error.message === 'Source workflow not found or access denied') {
        logger.warn(
          `[${requestId}] Access denied to source workflow ${sourceWorkflowId}`
        )
        return NextResponse.json({ error: 'Access denied' }, { status: 403 })
      }
    }
