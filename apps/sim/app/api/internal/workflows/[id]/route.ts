...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (isInternalCall) {
      ...
      const authResult = await authenticateApiKeyFromHeader(apiKeyHeader, { keyTypes: ['personal', 'workspace'] })
    ...
    if (normalizedData) {
      ...
      let deployedVersion: any = null
      // Include active deployed version basic info (not state), only if deployed
